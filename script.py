# Create a Pokemon-like game where your Classes could be different types of Creatures that can battle each other. 
# You could create a system where you have a Trainer class that has multiple Creatures. You could create Items to use on those Creatures.
class Pokemon:
    # To create a pokemon, give it a name, type, and level. Its max health is determined by its level. 
    ...
    def revive(self):
        ...
        self.is_knocked_out = False
        # A revived pokemon can't have 0 health. This is a safety precaution. revive() should only be called if the pokemon was given some health,
        # but if it somehow has no health, its health gets set to 1.
        if self.health == 0:
            self.health = 1
    ...
    def knock_out(self):
        ...
        # A knocked out pokemon can't have any health. This is a safety precaution. knock_out() should only be called if heath was set to 0, 
...
class Trainer:
    # A trainer has a list of pokemon, a number of potions and a name. When the trainer gets created, 
    # the first pokemon in their list of pokemons is the active pokemon (number 0)
